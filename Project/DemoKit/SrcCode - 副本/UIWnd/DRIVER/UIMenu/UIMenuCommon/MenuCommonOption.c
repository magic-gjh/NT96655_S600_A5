...
extern void FlowMovie_UpdateEdogMenuInfo(EDOG_INFO *pEdogInfo);
...
CTRL_LIST_ITEM(MenuCommonOption_ButtonMenu)
CTRL_LIST_ITEM(MenuCommonOption_ButtonUP)
CTRL_LIST_ITEM(MenuCommonOption_ButtonOK)
CTRL_LIST_ITEM(MenuCommonOption_ButtonDOWN)
CTRL_LIST_END
...
INT32 MenuCommonOption_OnEdogMessage(VControl *, UINT32, UINT32 *);
INT32 MenuCommonOption_TP(VControl *, UINT32, UINT32 *);
...
EVENT_ITEM(NVTEVT_EDOG_MESSAGE,MenuCommonOption_OnEdogMessage)
EVENT_ITEM(NVTEVT_KEY_TP,MenuCommonOption_TP)
EVENT_END
...
INT32 MenuCommonOption_TP(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
  UINT32 uiEvent;
    UINT32 P1,P2;
    UINT32  uiSoundMask;
	...
	switch(uiEvent)
	{
		...
		case GESTURE_RELEASE:
		    ...
		    if(TPIsOnRange(&MenuCommonOption_ButtonMenuCtrl,P1,P2)==TRUE)
		    	{
		    	debug_msg("^G  menu key\r\n");
		    	      Ux_PostEvent(NVTEVT_KEY_MENU, 1, NVTEVT_KEY_PRESS);	
								   
		    	}	
                    else  if(TPIsOnRange(&MenuCommonOption_ButtonUPCtrl,P1,P2)==TRUE)
    		    	{
    		    	debug_msg("^G  up key\r\n");
    		    	     Ux_PostEvent(NVTEVT_KEY_UP, 1, NVTEVT_KEY_PRESS);	
    		       	
    		    	}	
                    else  if(TPIsOnRange(&MenuCommonOption_ButtonOKCtrl,P1,P2)==TRUE)
    		    	{
    		    	debug_msg("^G  shu2 key\r\n");
    		    	     Ux_PostEvent(NVTEVT_KEY_SHUTTER2, 1, NVTEVT_KEY_PRESS);	
    		       	
    		    	}	
                    else  if(TPIsOnRange(&MenuCommonOption_ButtonDOWNCtrl,P1,P2)==TRUE)
		    	{
		    	debug_msg("^G  down key\r\n");
...
{
    Ux_CloseWindowClear(&MenuCommonItemCtrl,0); // close whole tab menu
    return NVTEVT_CONSUME;
}
...
INT32 MenuCommonOption_OnEdogMessage(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    UINT32  uiEvent;
    EDOG_INFO EdogInfo;
...
//----------------------MenuCommonOption_ButtonMenuCtrl Event---------------------------
EVENT_BEGIN(MenuCommonOption_ButtonMenu)
EVENT_END

//----------------------MenuCommonOption_ButtonUPCtrl Event---------------------------
EVENT_BEGIN(MenuCommonOption_ButtonUP)
EVENT_END

//----------------------MenuCommonOption_ButtonOKCtrl Event---------------------------
EVENT_BEGIN(MenuCommonOption_ButtonOK)
EVENT_END

//----------------------MenuCommonOption_ButtonDOWNCtrl Event---------------------------
EVENT_BEGIN(MenuCommonOption_ButtonDOWN)
EVENT_END
