...
void FlowPhoto_IconDrawStorage(VControl *pCtrl)
{
    ...
    if (UI_GetData(FL_CardStatus) == CARD_REMOVED)
        UxState_SetData(pCtrl,STATE_CURITEM,UIFlowWndPhoto_StatusICN_Storage_ICON_NULL);
...
{
    UxState_SetData(pCtrl,STATE_CURITEM,SysGetFlag(FL_FD));
    UxCtrl_SetShow(pCtrl,TRUE);
}
...
//        UxCtrl_SetShow(&UIFlowWndPhoto_PNL_FDFrameCtrl,FALSE);
    ...
    else
    {
       // FlowPhoto_IconDrawDSCMode(&UIFlowWndPhoto_StaticICN_DSCModeCtrl);
        FlowPhoto_IconDrawSize(&UIFlowWndPhoto_StaticTXT_SizeCCtrl);
        FlowPhoto_IconDrawQuality(&UIFlowWndPhoto_StatusICN_QualityCtrl);
        FlowPhoto_IconDrawAntishake(&UIFlowWndPhoto_StatusICN_AntiShakingCCtrl);
       // FlowPhoto_IconDrawFD(&UIFlowWndPhoto_StatusICN_FDCtrl);
        FlowPhoto_IconDrawFreePicNumber(&UIFlowWndPhoto_StaticTXT_FreePicCCtrl);
        ...
        FlowPhoto_IconDrawWB(&UIFlowWndPhoto_StatusICN_WBCCtrl);
        FlowPhoto_IconDrawEV(&UIFlowWndPhoto_StatusICN_EVCCtrl);
        FlowPhoto_IconDrawISO(&UIFlowWndPhoto_StatusICN_ISOCCtrl);
        FlowPhoto_IconDrawBattery(&UIFlowWndPhoto_StatusICN_BatteryCtrl);
        FlowPhoto_IconDrawDZoom(&UIFlowWndPhoto_StaticTXT_DZoomCtrl);
        UxCtrl_SetShow(&UIFlowWndPhoto_Status_ReturnCtrl,TRUE);
