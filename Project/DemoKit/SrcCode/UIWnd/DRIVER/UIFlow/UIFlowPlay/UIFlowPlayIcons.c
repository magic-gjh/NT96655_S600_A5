...
void FlowPB_IconDrawMovPlay(BOOL bShow)
{
    if (bShow==FALSE)
    {
        UxCtrl_SetShow(&UIFlowWndPlay_Status_PlayCtrl, FALSE);
	 return;
    }	
    if(g_PlbData.State == PLB_ST_PLAY_MOV)
    {
    	 UxState_SetData(&UIFlowWndPlay_Status_PlayCtrl,STATE_CURITEM,UIFlowWndPlay_Status_Play_ICON_PLAYBACK_PAUSE);    
        UxCtrl_SetShow(&UIFlowWndPlay_Status_PlayCtrl, TRUE);
    }
    else
    {
    	 UxState_SetData(&UIFlowWndPlay_Status_PlayCtrl,STATE_CURITEM,UIFlowWndPlay_Status_Play_ICON_PLAYBACK_PLAY);    
        UxCtrl_SetShow(&UIFlowWndPlay_Status_PlayCtrl, TRUE);
    }
}
...
void FlowPBUpdatePanelType(void)
{
    UINT32 uiFileFmt;
    uiFileFmt = AppPlay_GetData(PLAY_FILEFMT);
    debug_msg("UpdatePanelType:%x..\r\n",uiFileFmt);
	
    if(uiFileFmt & PBFMT_AVI || uiFileFmt & PBFMT_MOVMJPG)
        {
        SetIsMovieFile_Status(TRUE);	 	
        UxCtrl_SetShow(&UIFlowWndPlay_Panel_PlayMovieCtrl, TRUE);
        UxCtrl_SetShow(&UIFlowWndPlay_Panel_PlayPhotoCtrl, FALSE);
        }
     else
        {
        SetIsMovieFile_Status(FALSE);
        UxCtrl_SetShow(&UIFlowWndPlay_Panel_PlayMovieCtrl, FALSE);
        UxCtrl_SetShow(&UIFlowWndPlay_Panel_PlayPhotoCtrl, TRUE);
        }
