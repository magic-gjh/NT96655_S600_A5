...
#define ZOOM_LOCK_FLOW          DISABLE//ENABLE

#define ZOOM_EXTERNAL_W        119// 64
#define ZOOM_EXTERNAL_H        129// 48
...
INT32 UIFlowWndPlayMagnify_TP(VControl *, UINT32, UINT32 *);
...
EVENT_ITEM(NVTEVT_KEY_TP,UIFlowWndPlayMagnify_TP)
EVENT_END
...
INT32 UIFlowWndPlayMagnify_TP(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
  UINT32 uiEvent;
    UINT32 P1,P2;
    UINT32  uiSoundMask;
	...
	switch(uiEvent)
	{
		...
		case GESTURE_CKICK:
			...
			P2=paramArray[2];  
	            ...
	            if(TPIsOnRange(&UIFlowWndPlayMagnify_Status_PhotoZoominCtrl,P1,P2)==TRUE)
	                {
	                Ux_PostEvent(NVTEVT_KEY_ZOOMIN, 1, NVTEVT_KEY_PRESS);
	                }
                else if(TPIsOnRange(&UIFlowWndPlayMagnify_Status_PhotoZoomoutCtrl,P1,P2)==TRUE)
                    {
                    Ux_PostEvent(NVTEVT_KEY_ZOOMOUT, 1, NVTEVT_KEY_PRESS);
                    }
                     
                else if(TPIsOnRange(&UIFlowWndPlayMagnify_StaticIcon_DirUpCtrl,P1,P2)==TRUE)
                    {
                    Ux_PostEvent(NVTEVT_KEY_UP, 1, NVTEVT_KEY_PRESS);
                    }
                       
                else if(TPIsOnRange(&UIFlowWndPlayMagnify_StaticIcon_DirDownCtrl,P1,P2)==TRUE)
                    {
                    Ux_PostEvent(NVTEVT_KEY_DOWN, 1, NVTEVT_KEY_PRESS);
                    }
                else if(TPIsOnRange(&UIFlowWndPlayMagnify_StaticIcon_DirRightCtrl,P1,P2)==TRUE)
                    {
                    Ux_PostEvent(NVTEVT_KEY_RIGHT, 1, NVTEVT_KEY_PRESS);
                    }
                else if(TPIsOnRange(&UIFlowWndPlayMagnify_StaticIcon_DirLeftCtrl,P1,P2)==TRUE)
                    {
                    Ux_PostEvent(NVTEVT_KEY_LEFT, 1, NVTEVT_KEY_PRESS);
                    }
                else if(TPIsOnRange(&UIFlowWndPlayMagnify_Status_ReturnCtrl,P1,P2)==TRUE)
                    {
...
static void UIFlowWndPlayMagnify_DrawNavigatorWindow(void)
{
    ...
    UxCtrl_SetShow(&ZoomWinCtrl, FALSE);
