...
void Load_MenuInfo(void)
{
    ...
    if ((pSection = PStore_OpenSection(PS_SYS_PARAM, PS_RDWR)) != E_PS_SECHDLER)
    {
        ...
        {
            SetSPITPData(DEFAULT_TP_UI_MAX_X,DEFAULT_TP_UI_MIN_X,DEFAULT_TP_UI_MAX_Y,DEFAULT_TP_UI_MIN_Y);
            Reset_MenuInfo();
        }
        SetTPWindow(currentInfo.TP_UI_MAX_X,currentInfo.TP_UI_MIN_X,currentInfo.TP_UI_MAX_Y,currentInfo.TP_UI_MIN_Y);
...
void SetSPITPData(INT32 MAX_X,INT32 MIN_X,INT32 MAX_Y,INT32 MIN_Y)
{
    currentInfo.TP_UI_MAX_X = MAX_X;
    currentInfo.TP_UI_MIN_X = MIN_X;
    currentInfo.TP_UI_MAX_Y = MAX_Y;
    currentInfo.TP_UI_MIN_Y = MIN_Y;
}
...
#if 0//(_MODEL_DSC_ == _MODEL_PW8402_)
    //Edong flags
    SysLimitFlag(FL_EDOGMODE,        0,  EDOG_ID_MAX,   DEFAULT_EDOGMODE);
    SysLimitFlag(FL_EDOGVOLUME,        0,  VOLUME_MAX,   DEFAULT_EDOGVOLUME);
    SysLimitFlag(FL_EDOGMUTE,0,EDOG_MUTE_MAX,DEFAULT_EDOGMUTE);
    SysLimitFlag(FL_EDOGSENSITY,0,RADAR_SENSITIVITY_MAX,DEFAULT_EDOGSENSITY);
    SysLimitFlag(FL_EDOGDRIVEMODE,0,DRIVERMODE_MAX,DEFAULT_EDOGDRIVEMODE);	
    SysLimitFlag(FL_EDOGSPEED_LIMIT,0,SPEEDLIMIT_MAX,DEFAULT_EDOGSPEED_LIMIT);	
    SysLimitFlag(FL_EDOGSPEED_ADJUST,0,SPEED_ADJUST_MAX,DEFAULT_EDOGSPEED_ADJUST);	
    SysLimitFlag(FL_EDOGBROADCAST,0,BROADCAST_MAX,DEFAULT_EDOGBROADCAST);		
    SysLimitFlag(FL_EDOG_WRNINGINDEX,0,EDOG_MSG_MAX,DEFAULT_EDOGWARNING);			
#endif	
    SysLimitFlag(FL_EDOG_WRNINGINDEX,0,EDOG_MSG_MAX,DEFAULT_EDOGWARNING);		
...
void SysSetFixedFlag(void)
{
    ...
    SysSetFlag(FL_EDOGMODE, DEFAULT_EDOGMODE);
...
//    SysSetFlag(FL_MOVIE_EV,             DEFAULT_MOVIE_EV);
    ...
    SysSetFlag(FL_EDOG_WRNINGINDEX, DEFAULT_EDOGWARNING);

    UI_SetData(FL_USER_SPEED_LIMIT,USER_SPEED_LIMIT_OFF);
    UI_SetData(FL_CITYMODE_SPEEDLIMIT,CITYMODE_SPEEDLIMIT_60);
    UI_SetData(FL_RADAR_REPORT,RADAR_REPORT_ON);
    UI_SetData(FL_RADAR_K_REPORT,1);
    UI_SetData(FL_RADAR_KA_REPORT,1);
    UI_SetData(FL_RADAR_X_REPORT,RADAR_X_REPORT_OFF);
    UI_SetData(FL_DRIVING_MODE,DRIVING_MODE_ALL);
    UI_SetData(FL_SPEED_Correction,SPEED_Correction_OFF);
    UI_SetData(FL_RADAR_ONOFF,RADAR_SET_ON);



    //sprintf(&Data,"%x",USER_SPEED_LIMIT_OFF+7);
    RD_PutCommand(USER_SPEED_LIMIT, USER_SPEED_LIMIT_OFF+7);

    Delay_DelayMs(50);
    //sprintf(&Data,"%x",CITYMODE_SPEEDLIMIT_60+2);
    RD_PutCommand(CITYMODE_SPEEDLIMIT, CITYMODE_SPEEDLIMIT_60+2);

    Delay_DelayMs(50);
    //sprintf(&Data,"%x",RADAR_REPORT_ON);
    RD_PutCommand(RADAR_REPORT, RADAR_REPORT_ON);
    
    Delay_DelayMs(50);
    //sprintf(&Data,"%x",RADAR_X_REPORT_OFF);
    RD_PutCommand(0x56, RADAR_X_REPORT_OFF);
    
    Delay_DelayMs(50);
    //sprintf(&Data,"%x",DRIVING_MODE_ALL+5);
    RD_PutCommand(DRIVING_MODE, DRIVING_MODE_ALL+5);
    
    Delay_DelayMs(50);
    //sprintf(&Data,"%x",SPEED_Correction_OFF);
    RD_PutCommand(SPEED_Correction, SPEED_Correction_OFF);

    Delay_DelayMs(50);
    //sprintf(&Data,"%x",RADAR_SET_ON);
    RD_PutCommand(RADAR_ONOFF, RADAR_SET_ON);
...
void FlowDCF_UpdateName(void)
{
 ...
 CHAR filePath[DCF_FULL_FILE_PATH_LEN];
    ...
    curDate = rtc_getDate();
    curTime = rtc_getTime();
	//debug_msg("magic_rtc_getDate: year = %4d, month = %d, day = %d\r\n",curDate.s.year,curDate.s.month,curDate.s.day);
...
#define CALIBRATION_X_RANG    25

int TPIsRangePossition(VControl *pCtrl,UINT32 P1,UINT32 P2)
{
 	Ux_RECT Rect;
	...
	//Rect.x1-= CALIBRATION_X_RANG;	
        ...
        if((P1>Rect.x1)&&(P1<Rect.x2)&&(P2>(Rect.y1+(Rect.y2-Rect.y1)/2))&&(P2<Rect.y2))
            {
            return TP_PRESS_DOWN;
            }
        else if((P1>Rect.x1)&&(P1<Rect.x2)&&(P2<(Rect.y2-(Rect.y2-Rect.y1)/2))&&(P2>Rect.y1))
            {
            return TP_PRESS_UP;
            }
        else
