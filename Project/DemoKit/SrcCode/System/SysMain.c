...
//-------jacky lan --------------
	...
	if(((GxSystem_GetPowerOnSource() == GX_PWRON_SRC_PWR_VBUS)&&(usb_StateChange()==TRUE))||(gBinInfo.ld.LdCtrl2 & LDCF_UPDATE_FW)||(GxSystem_GetPowerOnSource() == GX_PWRON_SRC_PWR_SW2))
	{	   
		usb_close();		
	       System_OnVideoInit();
	}
