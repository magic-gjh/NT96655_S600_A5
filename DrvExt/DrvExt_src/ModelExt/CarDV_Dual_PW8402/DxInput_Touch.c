...
#if 0
//for SHX 3.0 Panel
#define TP_HW_MAX_X        980
#define TP_HW_MAX_Y        762//928
#define TP_HW_MIN_X        70//63
#define TP_HW_MIN_Y        220//82

#else
//for S600  Panel
/*
#define TP_HW_MAX_X        960
#define TP_HW_MAX_Y        928
#define TP_HW_MIN_X        76//63
#define TP_HW_MIN_Y        92//82
*/
static INT32 TP_HW_MAX_X = 973;//960;
static INT32 TP_HW_MAX_Y    =    947;//928;
static INT32 TP_HW_MIN_X     =   34;//76;
static INT32 TP_HW_MIN_Y    =    92;
#endif

void SetTPWindow(INT32 MAX_X,INT32 MIN_X,INT32 MAX_Y,INT32 MIN_Y)
{
    TP_HW_MAX_X = MAX_X;
    TP_HW_MAX_Y = MAX_Y;
    TP_HW_MIN_X = MIN_X;
...
void GetTPWindow(INT32* MAX_X,INT32* MIN_X,INT32* MAX_Y,INT32* MIN_Y)
{
    *MAX_X = TP_HW_MAX_X;
    *MAX_Y = TP_HW_MAX_Y;
    *MIN_X = TP_HW_MIN_X;
...
#define TP_PANEL_X2          799//319
#define TP_PANEL_Y2          479//239
...
static UINT32 guiCaculateY_Offset;
...
static void DetTP_Convert2RealXY(INT32 *pX, INT32 *pY)
{
 ...
 if (0)
    ...
    *pY =  TP_PANEL_Y1 + TP_PANEL_H*(TP_HW_MAX_Y - tempY)/(TP_HW_MAX_Y - TP_HW_MIN_Y);
    ...
    if(TP_PANEL_X2 <(  TP_PANEL_W*(tempX - TP_HW_MIN_X)/(TP_HW_MAX_X - TP_HW_MIN_X)) )
        *pX = 0;
        else
    *pX =TP_PANEL_X2 -(  TP_PANEL_W*(tempX - TP_HW_MIN_X)/(TP_HW_MAX_X - TP_HW_MIN_X));
        /*
    if(TP_PANEL_Y2 <(  TP_PANEL_H*(tempY - TP_HW_MIN_Y)/(TP_HW_MAX_Y - TP_HW_MIN_Y)) )
        *pY = 0;
        else
    *pY =TP_PANEL_Y2 -(  TP_PANEL_H*(tempY - TP_HW_MIN_Y)/(TP_HW_MAX_Y - TP_HW_MIN_Y));
    */
    //*pX =  TP_PANEL_X1 + TP_PANEL_W*(tempX - TP_HW_MIN_X)/(TP_HW_MAX_X - TP_HW_MIN_X);
        *pY =  TP_PANEL_H*(tempY - TP_HW_MIN_Y)/(TP_HW_MAX_Y - TP_HW_MIN_Y);
    }
    //debug_msg("Convert:(%d,%d) to (%d,%d)\r\n",tempX,tempY,*pX,*pY);
...
#define POSITION_RANG     (5)
static UINT32 TP_X = 0;
static UINT32 TP_Y = 0;

void GetCurTPXY(UINT32 *pX, UINT32 *pY)
{
    *pX = TP_X;
    *pY = TP_Y;
}
...
void DetTP_GetXY(INT32 *pX, INT32 *pY)
{
    ...
    static UINT32 X_Pre=0XFFFF,Y_Pre=0XFFFF;
    temp = 0 ;
    xsum = ysum = 0;
